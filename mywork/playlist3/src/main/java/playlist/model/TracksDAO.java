...
public class TracksDAO extends CassandraData {
  ...
  public void star() {
    ...
    PreparedStatement preparedStatement =
    getSession().prepare("UPDATE track_by_artist  SET withstar = ? WHERE artist = ? AND track = ? AND track_id = ?");
	BoundStatement boundStatement = preparedStatement.bind(true, this.artist,this.track,this.track_id);
	getSession().execute(boundStatement);
	
   preparedStatement = getSession().prepare("UPDATE track_by_id  SET withstar = ? WHERE track_id = ?");
   boundStatement = preparedStatement.bind(true, this.track_id);
   getSession().execute(boundStatement);
			
   preparedStatement = getSession().prepare("UPDATE track_by_genre  SET withstar = ? WHERE genre = ? AND artist = ? AND track = ? AND track_id = ?");
   boundStatement = preparedStatement.bind(true, this.genre, this.artist, this.track, this.track_id);
   getSession().execute(boundStatement);
